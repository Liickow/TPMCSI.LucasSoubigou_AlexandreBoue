...
def visualize(
  ...
  for detection in detection_result.detections:
    ...
    gauche = detection.keypoints[0]
    droite = detection.keypoints[1]
    kpGauche = _normalized_to_pixel_coordinates(gauche.x,gauche.y, width, height)
    kpDroite = _normalized_to_pixel_coordinates(droite.x, droite.y, width, height)

    # Draw the center of the eyes with a different color
    colorGauche, colorDroite, thickness, radius = (255,0,0), (0,255,0), 2, 2
    cv2.circle(annotated_image, kpGauche, thickness, colorGauche, radius)
    cv2.circle(annotated_image, kpDroite, thickness, colorDroite, radius)
	...
	while True:
		...
		if res.tracking_results != None:
			...
			for detection in res.tracking_results.detections :
				if detection.bounding_box.width * detection.bounding_box.height > sizeBigger or bigger == None:
					bigger = detection
					sizeBigger = detection.bounding_box.width * detection.bounding_box.height
			...
			if bigger != None:
				pxXG = bigger.keypoints[0].x * frame_width
				pxYG = bigger.keypoints[0].y * frame_height
				pxXD = bigger.keypoints[1].x * frame_width
				pxYD = bigger.keypoints[1].y * frame_height
				pxXMid = (pxXG + pxXD) / 2
				pxYMid = (pxYG + pxYD) / 2
				print("Pos G",pxXG," ",pxYG," Pos D",pxXD," ",pxYD," Pos Mid",pxXMid," ",pxYMid)

					# compute the interpupillary distance (in pixels)
				distanceX = abs(pxXD - pxXG)
				distanceY = abs(pxYD - pxYG)
				print("distances : ",distanceX, distanceY, math.sqrt(distanceX**2 + distanceY**2))
